...
use submodules::{boundary_conditions::Spherical, configurations::Configuration, energy_evaluation::{ELJEven, PotentialKinds}, ensembles::{EnsembleTrait, NVT}, input_params::{MCParams, TempGrid}, ptmc::PTMC, system_solver::SystemSolver, type_lib::NumericData};
...
fn main() {
    ...
    let energy_vector = ptmc.results.readfile();
    let (processed_histogram, processed_energy_vector, sums) = ptmc.results.process_histogram(&energy_vector);
    println!("dim of hist: {:?}", processed_histogram.dim());
    println!("dim of energy: {:?}", processed_energy_vector.len());
    let sys_solver = SystemSolver::multihistogram(&processed_histogram, &processed_energy_vector, &ptmc.temp_grid.beta_grid, &sums);
    println!("{}", sys_solver.c_v);
